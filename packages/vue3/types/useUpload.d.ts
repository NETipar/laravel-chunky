...
/**
 * Polymorphic upload composable. Accepts either a single `File` or
 * `File[]` — internally uses `useBatchUpload` because every upload is
 * a batch of N files (single file = batch of 1). The signature matches
 * the v1.0 canonical API.
 *
 * Prefer this over `useBatchUpload` / `useChunkUpload` for new code.
 * The two specific composables remain for back-compat.
 */
